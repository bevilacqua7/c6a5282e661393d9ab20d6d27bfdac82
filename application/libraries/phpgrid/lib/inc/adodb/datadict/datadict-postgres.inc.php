...
class ADODB2_postgres extends ADODB_DataDict {
	...
	function MetaType($t, $len = -1, $fieldobj = false) {
		...
		$is_serial = is_object ( $fieldobj ) && ! empty ( $fieldobj->primary_key ) && ! empty ( $fieldobj->unique ) && ! empty ( $fieldobj->has_default ) && substr ( $fieldobj->default_value, 0, 8 ) == 'nextval(';
	 ...
	 *        	table-name
	 * @param string $flds
	 *        	column-names and types for the changed columns
	...
	function DropIndexSQL($idxname, $tabname = NULL) {
		return array (
				sprintf ( $this->dropIndex, $this->TableName ( $idxname ), $this->TableName ( $tabname ) ) 
		);
	}
	...
	/**
	 * Save the content into a temp.
	 * table, drop and recreate the original table and copy the content back in
	 *
	...
	function _recreate_copy_table($tabname, $dropflds, $tableflds, $tableoptions = '') {
		...
		foreach ( $this->MetaColumns ( $tabname ) as $fld ) {
			if (! $dropflds || ! in_array ( $fld->name, $dropflds )) {
				...
				if (preg_match ( '/' . $fld->name . ' (I|I2|I4|I8|N|F)/i', $tableflds, $matches ) && in_array ( $fld->type, array (
						'varchar',
						'char',
						'text',
						'bytea' 
				...
				if ($fld->primary_key && $fld->has_default && preg_match ( "/nextval\('([^']+)'::text\)/", $fld->default_value, $matches )) {
					$seq_name = $matches [1];
					$seq_fld = $fld->name;
				}
			}
		...
		foreach ( $this->MetaIndexes ( $tabname ) as $idx_name => $idx_data ) {
			if (substr ( $idx_name, - 5 ) != '_pkey' && (! $dropflds || ! count ( array_intersect ( $dropflds, $idx_data ['columns'] ) ))) {
				$aSql = array_merge ( $aSql, $this->CreateIndexSQL ( $idx_name, $tabname, $idx_data ['columns'], $idx_data ['unique'] ? array (
						'UNIQUE' 
				) : False ) );
			}
		}
	...
	function RenameTableSQL($tabname, $newname) {
		if (! empty ( $this->schema )) {
			...
			return array (
					sprintf ( $this->renameTable, $rename_from, $rename_to ) 
			);
		}
		
		return array (
				sprintf ( $this->renameTable, $this->TableName ( $tabname ), $this->TableName ( $newname ) ) 
		);
	}
