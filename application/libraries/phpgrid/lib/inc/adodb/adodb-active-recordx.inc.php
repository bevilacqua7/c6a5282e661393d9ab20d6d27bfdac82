...
class ADODB_Active_Record {
	...
	function LoadRelations($name, $whereOrderBy, $offset = -1, $limit = -1) {
		...
		if (! empty ( $table->_belongsTo [$name] )) {
			...
			if (empty ( $this->$columnName )) {
				...
				$arrayOfOne = $obj->Find ( $belongsToId . '=' . $this->$columnName . ' ' . $whereOrderBy, false, false, $extras );
		...
		if (! empty ( $table->_hasMany [$name] )) {
			...
			$this->$name = $obj->Find ( $obj->foreignKey . '=' . $this->$hasManyId . ' ' . $whereOrderBy, false, false, $extras );
			return $this->$name;
		}
	}
	// ////////////////////////////////
	...
	function doquote(&$db, $val, $t) {
		switch ($t) {
			...
			case 'X' :
				...
				if (strlen ( $val ) > 0 && (strncmp ( $val, "'", 1 ) != 0 || substr ( $val, strlen ( $val ) - 1, 1 ) != "'")) {
					return $db->qstr ( $val );
					break;
				}
			default :
	...
	function Load($where = null, $bindarr = false) {
		...
		foreach ( $table->_belongsTo as $foreignTable ) {
			...
			$qry .= ' LEFT JOIN ' . $foreignTable->_table . ' ON ' . $this->_table . '.' . $foreignTable->foreignKey . '=' . $foreignTable->_table . '.' . $belongsToId;
		}
		foreach ( $table->_hasMany as $foreignTable ) {
			$qry .= ' LEFT JOIN ' . $foreignTable->_table . ' ON ' . $this->_table . '.' . $hasManyId . '=' . $foreignTable->_table . '.' . $foreignTable->foreignKey;
		}
		if ($where) {
			$qry .= ' WHERE ' . $where;
		}
	...
	function Find($whereOrderBy, $bindarr = false, $pkeysArr = false, $extra = array()) {
		...
		$table = & $this->TableInfo ();
		$arr = $db->GetActiveRecordsClass ( get_class ( $this ), $this, $whereOrderBy, $bindarr, $pkeysArr, $extra, array (
	...
	if (ADODB_JOIN_AR == $extra ['loading']) {
		if (! empty ( $relations ['belongsTo'] )) {
			foreach ( $relations ['belongsTo'] as $foreignTable ) {
				...
				$qry .= ' LEFT JOIN ' . $foreignTable->_table . ' ON ' . $table . '.' . $foreignTable->foreignKey . '=' . $foreignTable->_table . '.' . $belongsToId;
			}
		}
		if (! empty ( $relations ['hasMany'] )) {
			...
			foreach ( $relations ['hasMany'] as $foreignTable ) {
				$qry .= ' LEFT JOIN ' . $foreignTable->_table . ' ON ' . $table . '.' . $hasManyId . '=' . $foreignTable->_table . '.' . $foreignTable->foreignKey;
			}
		}
	}
