 ...
 * @return s 2 dimensional array containing parsed tokens.
 *        
 */
...
class ADODB_DataDict {
	...
	function DropIndexSQL($idxname, $tabname = NULL) {
		return array (
				sprintf ( $this->dropIndex, $this->NameQuote ( $idxname ), $this->TableName ( $tabname ) ) 
		);
	}
	...
	function RenameColumnSQL($tabname, $oldcolumn, $newcolumn, $flds = '') {
		...
		return array (
				sprintf ( $this->renameColumn, $tabname, $this->NameQuote ( $oldcolumn ), $this->NameQuote ( $newcolumn ), $column_def ) 
		);
	}
	...
	function DropTableSQL($tabname) {
		return array (
				sprintf ( $this->dropTable, $this->TableName ( $tabname ) ) 
		);
	}
		...
		foreach ( $flds as $fld ) {
			...
			if ($fdefts) {
				...
				if ($ty == 'C' or $ty == 'X' or (substr ( $fdefault, 0, 1 ) != "'" && ! is_numeric ( $fdefault ))) {
					...
					} else if (strlen ( $fdefault ) != 1 && substr ( $fdefault, 0, 1 ) == ' ' && substr ( $fdefault, strlen ( $fdefault ) - 1 ) == ' ')
