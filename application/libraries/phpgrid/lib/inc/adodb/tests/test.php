...
*/
  ...
  KEY `myindex` (`row1`,`row2`)
	...
	if ($rs && ! $rs->EOF) {
		...
		if ($arr [strtoupper ( $id )] != 1) {
			Err ( "Error 1" );
			print_r ( $arr );
		}
		;
		if (trim ( $arr [strtoupper ( $fname )] ) != 'Caroline') {
			Err ( "Error 2" );
			print_r ( $arr );
		}
		;
	...
	for($loop = 0; $loop < 1; $loop ++) {
		...
		$rs = $db->CacheExecute ( 4, "select distinct firstname,lastname from ADOXYZ" );
		if ($rs) {
			print ' Multiple, Alan and George selected: ' . $rs->GetMenu ( 'menu', array (
					'Alan',
					'George' 
			), false, true );
			if (empty ( $rs->connection ))
	...
	$db->debug = true;
	$sql = PivotTableSQL ( $db, // adodb connection
'ADOXYZ', // tables
...
<body bgcolor=white>
	...
	This script tests the following databases: Interbase, Oracle, Visual
	FoxPro, Microsoft Access (ODBC and ADO), MySQL, MSSQL (ODBC, native,
	ADO). There is also support for Sybase, PostgreSQL.
	</p>
	For the latest version of ADODB, visit
	<a href=http://adodb.sourceforge.net />adodb.sourceforge.net
	</a>.
	</p>

	Test
	<a href=test4.php>GetInsertSQL/GetUpdateSQL</a> &nbsp;
	<a href=testsessions.php>Sessions</a> &nbsp;
	<a href=testpaging.php>Paging</a> &nbsp;
	<a href=test-perf.php>Perf Monitor</a>
	<p>
<?php
...
?>
	...
	<p>
		<i>ADODB Database Library  (c) 2000-2014 John Lim. All rights reserved. Released under BSD and LGPL, PHP <?php echo PHP_VERSION ?>.</i>
	</p>
</body>
</html>
