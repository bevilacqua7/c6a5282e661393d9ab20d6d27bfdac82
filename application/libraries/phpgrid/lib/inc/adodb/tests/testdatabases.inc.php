...
<table>
	<tr valign=top>
		<td>
			<form method=get>
				<input type=checkbox name="testaccess" value=1
					<?php echo !empty($testaccess) ? 'checked' : '' ?>> <b>Access</b><br>
				<input type=checkbox name="testibase" value=1
					<?php echo !empty($testibase) ? 'checked' : '' ?>> <b>Interbase</b><br>
				<input type=checkbox name="testmssql" value=1
					<?php echo !empty($testmssql) ? 'checked' : '' ?>> <b>MSSQL</b><br>
				<input type=checkbox name="testmysql" value=1
					<?php echo !empty($testmysql) ? 'checked' : '' ?>> <b>MySQL</b><br>
				<input type=checkbox name="testmysqlodbc" value=1
					<?php echo !empty($testmysqlodbc) ? 'checked' : '' ?>> <b>MySQL
					ODBC</b><br> <input type=checkbox name="testmysqli" value=1
					<?php echo !empty($testmysqli) ? 'checked' : '' ?>> <b>MySQLi</b> <br>
				<td><input type=checkbox name="testsqlite" value=1
					<?php echo !empty($testsqlite) ? 'checked' : '' ?>> <b>SQLite</b><br>
					<input type=checkbox name="testproxy" value=1
					<?php echo !empty($testproxy) ? 'checked' : '' ?>> <b>MySQL Proxy</b><br>
					<input type=checkbox name="testoracle" value=1
					<?php echo !empty($testoracle) ? 'checked' : '' ?>> <b>Oracle
						(oci8)</b> <br> <input type=checkbox name="testpostgres" value=1
					<?php echo !empty($testpostgres) ? 'checked' : '' ?>> <b>PostgreSQL</b><br>
					<input type=checkbox name="testpostgres9" value=1
					<?php echo !empty($testpostgres9) ? 'checked' : '' ?>> <b>PostgreSQL
						9</b><br> <input type=checkbox name="testpgodbc" value=1
					<?php echo !empty($testpgodbc) ? 'checked' : '' ?>> <b>PostgreSQL
						ODBC</b><br>
				
				<td><input type=checkbox name="testpdopgsql" value=1
					<?php echo !empty($testpdopgsql) ? 'checked' : '' ?>> <b>PgSQL PDO</b><br>
					<input type=checkbox name="testpdomysql" value=1
					<?php echo !empty($testpdomysql) ? 'checked' : '' ?>> <b>MySQL PDO</b><br>
					<input type=checkbox name="testpdosqlite" value=1
					<?php echo !empty($testpdosqlite) ? 'checked' : '' ?>> <b>SQLite
						PDO</b><br> <input type=checkbox name="testpdoaccess" value=1
					<?php echo !empty($testpdoaccess) ? 'checked' : '' ?>> <b>Access
						PDO</b><br> <input type=checkbox name="testpdomssql" value=1
					<?php echo !empty($testpdomssql) ? 'checked' : '' ?>> <b>MSSQL PDO</b><br>

					<input type=checkbox name="testpdoora" value=1
					<?php echo !empty($testpdoora) ? 'checked' : '' ?>> <b>OCI PDO</b><br>
				
				<td><input type=checkbox name="testdb2" value=1
					<?php echo !empty($testdb2) ? 'checked' : '' ?>> DB2<br> <input
					type=checkbox name="testvfp" value=1
					<?php echo !empty($testvfp) ? 'checked' : '' ?>> VFP+ODBTP<br> <input
					type=checkbox name="testado" value=1
					<?php echo !empty($testado) ? 'checked' : '' ?>> ADO (for mssql and
					access)<br> <input type=checkbox name="nocountrecs" value=1
					<?php echo !empty($nocountrecs) ? 'checked' : '' ?>>
					$ADODB_COUNTRECS=false<br> <input type=checkbox name="nolog"
					value=1 <?php echo !empty($nolog) ? 'checked' : '' ?>> No SQL
					Logging<br> <input type=checkbox name="time" value=1
					<?php echo !empty($_GET['time']) ? 'checked' : '' ?>> ADOdb time
					test
...
if ($ADODB_FETCH_MODE != ADODB_FETCH_DEFAULT)
	print "<h3>FETCH MODE IS NOT ADODB_FETCH_DEFAULT</h3>";

if (isset ( $nocountrecs ))
...
$pg_errmsg = "ERROR: PostgreSQL requires a database called '$pg_database' " . "on server '$pg_hostname', user '$pg_user', password '$pg_password'.<BR>";
...
if (! empty ( $testpgodbc )) {
	...
	if ($db->PConnect ( 'Postgresql' )) {
		...
		testdb ( $db, "create table ADOXYZ (id int, firstname char(24), lastname char(24), created date) type=innodb" );
	} else
...
if (! empty ( $testibase )) {
	...
	if ($db->PConnect ( "localhost:d:\\firebird\\151\\examples\\EMPLOYEE.fdb", "sysdba", "masterkey", "" ))
		...
		print "ERROR: Interbase test requires a database called employee.gdb" . '<BR>' . $db->ErrorMsg ();
}
...
if (! empty ( $testpdopgsql )) {
	...
	testdb ( $db, "create table ADOXYZ (id int, firstname char(24), lastname char(24), created date)" );
}
...
if (! empty ( $testaccess )) {
	...
	if ($db->PConnect ( $dsn, "", "", "" ))
		...
		print "ERROR: Access test requires a Windows ODBC DSN=nwind, Access driver";
}
...
if (! empty ( $testvfp )) { // ODBC
	...
	if ($db->PConnect ( "vfp-adoxyz" )) {
		...
		print "ERROR: Visual FoxPro test requires a Windows ODBC DSN=vfp-adoxyz, VFP driver";
	...
	if ($db->PConnect ( 'localhost', 'DRIVER={Microsoft Visual FoxPro Driver};SOURCETYPE=DBF;SOURCEDB=d:\inetpub\adodb;EXCLUSIVE=NO;' )) {
		...
		print "ERROR: Visual FoxPro odbtp requires a Windows ODBC DSN=vfp-adoxyz, VFP driver";
}

// REQUIRES MySQL server at localhost with database 'test'
if (! empty ( $testmysql )) { // MYSQL
	
	if (PHP_VERSION >= 5 || $_SERVER ['HTTP_HOST'] == 'localhost')
		$server = 'localhost';
	else
		$server = "mangrove";
	$user = 'root';
	...
	if (true || $db->PConnect ( $server, "root", "", "northwind" )) {
		...
		print "ERROR: MySQL test requires a MySQL server on localhost, userid='admin', password='', database='test'" . '<BR>' . $db->ErrorMsg ();
}
...
if (false && ! empty ( $testoracle )) {
	...
	if ($db->PConnect ( "", "scott", "tiger", "natsoft.domain" ))
		...
		print "ERROR: Oracle test requires an Oracle server setup with scott/tiger" . '<BR>' . $db->ErrorMsg ();
}
...
if (false && ! empty ( $testmssql )) { // MS SQL Server -- the extension is buggy -- probably better to use ODBC
	...
	if ($ok /*or $db->PConnect("mangrove", "sa", "natsoft", "ai")*/) {
		...
		print "ERROR: MSSQL test 2 requires a MS SQL 7 on a server='$server', userid='adodb', password='natsoft', database='ai'" . '<BR>' . $db->ErrorMsg ();
}
