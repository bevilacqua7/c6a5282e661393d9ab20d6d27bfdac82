...
class adodb_perf {
	...
	function Tracer($sql) {
		...
		$arr = $this->conn->GetArray ( "select count(*),tracer
	...
	function _SuspiciousSQL($numsql = 10) {
		...
		if ($this->conn->fetchMode !== false)
			$savem = $this->conn->SetFetchMode ( false );
			// $this->conn->debug=1;
		$rs = $this->conn->SelectLimit ( "select avg(timer) as avg_timer,$sql1,count(*),max(timer) as max_timer,min(timer) as min_timer
				from $perf_table
...
<table border=1 bgcolor=white><tr><td><b>Load</b><td><b>Count</b><td><b>SQL</b><td><b>Max</b><td><b>Min</b></tr>\n";
	...
	function UI($pollsecs = 5) {
		...
		if (isset ( $_GET ['sql'] ) && get_magic_quotes_gpc ()) {
			$_GET ['sql'] = $_GET ['sql'] = str_replace ( array (
					"\\'",
					'\"' 
			), array (
					"'",
