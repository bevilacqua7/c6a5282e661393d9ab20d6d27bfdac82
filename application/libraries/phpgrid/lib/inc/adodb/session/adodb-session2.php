...
class ADODB_Session {
	...
	static function write($key, $oval) {
		...
		if (! $clob) { // no lobs, simply use replace()
			$rs = $conn->Execute ( "SELECT COUNT(*) AS cnt FROM $table WHERE $binary sesskey = " . $conn->Param ( 0 ), array (
					$key 
			) );
			if ($rs)
				$rs->Close ();
	...
	static function gc($maxlifetime) {
		...
		if ($rs) {
			...
			while ( ! $rs->EOF ) {
				...
				if ($fn)
					$fn ( $ref, $key );
				$del = $conn->Execute ( "DELETE FROM $table WHERE sesskey=" . $conn->Param ( '0' ), array (
						$key 
				) );
				$rs->MoveNext ();
				$ccnt += 1;
