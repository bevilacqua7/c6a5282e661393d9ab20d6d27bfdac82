...
";
	 ...
	 *
	 * @param string|false $argHostname
	 *        	DB server hostname or TNS name
	 * @param string $argUsername        	
	 * @param string $argPassword        	
	 * @param string $argDatabasename
	 *        	Service name, SID (defaults to null)
	 * @param int $mode
	 *        	Connection mode, defaults to 0
	...
	function _connect($argHostname, $argUsername, $argPassword, $argDatabasename = null, $mode = 0) {
		...
		if ($mode == 1) {
			$this->_connectionID = ($this->charSet) ? oci_pconnect ( $argUsername, $argPassword, $argDatabasename, $this->charSet ) : oci_pconnect ( $argUsername, $argPassword, $argDatabasename );
			if ($this->_connectionID && $this->autoRollback) {
				oci_rollback ( $this->_connectionID );
			}
		} else if ($mode == 2) {
			$this->_connectionID = ($this->charSet) ? oci_new_connect ( $argUsername, $argPassword, $argDatabasename, $this->charSet ) : oci_new_connect ( $argUsername, $argPassword, $argDatabasename );
		} else {
			$this->_connectionID = ($this->charSet) ? oci_connect ( $argUsername, $argPassword, $argDatabasename, $this->charSet ) : oci_connect ( $argUsername, $argPassword, $argDatabasename );
		}
	...
	function Prepare($sql, $cursor = false) {
		...
		if ($sttype == 'BEGIN' || $sttype == 'DECLARE') {
			return array (
					...
					0,
					$BINDNUM,
					($cursor) ? oci_new_cursor ( $this->_connectionID ) : false 
			);
		}
		return array (
				$sql,
	...
	function ExecuteCursor($sql, $cursorName = 'rs', $params = false) {
		if (is_array ( $sql )) {
			...
			$stmt = ADODB_oci8::Prepare ( $sql, true ); // true to allocate oci_new_cursor
	 ...
	 *        	returned by Prepare() or PrepareSP().
	 * @param $var PHP
	 *        	variable to bind to
	 * @param $name Name
	 *        	of stored procedure variable name to bind to.
	 * @param
...
SELECT /*+ RULE */ distinct b.column_name
  ...
  WHERE ( UPPER(b.table_name) = ('$table'))
	 ...
	 * @param str $owner        	
	 * @param bool $upper        	
	 * @return mixed[] of foreign key information
	 *        
	 ...
	 *        	if $s is GET/POST var, set to get_magic_quotes_gpc().
...
class ADORecordset_oci8 extends ADORecordSet {
	 ...
	 * memory while your script is running.
	 * All associated result memory for the
	 ...
	 *        	of blobsize
	 * @param bool $fieldobj        	
	 * @return str metatype of the field
	 */
