	...
	class ADODB_text extends ADOConnection {
		...
		function Connect(&$array, $types = false, $colnames = false) {
			...
			if (! $types) {
				...
				$types = array ();
				$firstrow = true;
				if ($this->_proberows > sizeof ( $array ))
					$max = sizeof ( $array );
				else
					$max = $this->_proberows;
		...
		function _query($sql, $input_arr, $eval = false) {
			...
			if (sizeof ( $arr ) < 2)
				return $this; // actually invalid sql
			$col = $arr [1];
			$at = ( integer ) $col;
			if ($at == 0) {
				$i = 0;
			...
			if ($at <= 0 || $at > sizeof ( $projarray [0] ))
				return $this; // cannot find sort column
			$at -= 1;
