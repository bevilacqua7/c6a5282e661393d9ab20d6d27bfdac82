...
if (! defined ( "_ADODB_CSV_LAYER" )) {
	...
	class ADODB_csv extends ADOConnection {
		...
		function SelectLimit($sql, $nrows = -1, $offset = -1, $inputarr = false, $secs2cache = 0) {
			...
			$url = $this->_url . '?sql=' . urlencode ( $sql ) . "&nrows=$nrows&fetch=" . (($this->fetchMode !== false) ? $this->fetchMode : $ADODB_FETCH_MODE) . "&offset=$offset";
			$err = false;
		...
		function _Execute($sql, $inputarr = false) {
			...
			$url = $this->_url . '?sql=' . urlencode ( $sql ) . "&fetch=" . (($this->fetchMode !== false) ? $this->fetchMode : $ADODB_FETCH_MODE);
			$err = false;
