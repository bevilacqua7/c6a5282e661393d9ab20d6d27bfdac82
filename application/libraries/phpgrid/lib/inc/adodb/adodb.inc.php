...
$ADODB_FETCH_MODE, $ADODB_CACHE, $ADODB_CACHE_CLASS, $ADODB_FORCE_TYPE, $ADODB_GETONE_EOF, $ADODB_QUOTE_FIELDNAMES;
	...
	abstract class ADOConnection {
		 ...
		 *
		 * @return s An array with 2 elements: $arr['string'] is the description string,
		 ...
		 *        	table to lock
		 * @param $where where
		 *        	to use, eg: "WHERE row=12". If left empty, will escalate to table lock
		 */
		 ...
		 * e.g.
		 * $DB->Param('a') will return
		 * - '?' for most databases
		 * - ':a' for Oracle
		 * - '$1', '$2', etc. for PostgreSQL
		 * 
		 * @param string $name
		 *        	parameter's name, false to force a reset of the
		 ...
		 * 
		 * @return s true on commit, false on rollback.
		 */
			...
			if ($inputarr !== false) {
				...
				if (! is_array ( $sql ) && ! $this->_bindInputArray) {
					...
					if ($nparams != count ( $inputarr )) {
						$this->outp_throw ( "Input array has " . count ( $inputarr ) . " params, does not match query: '" . htmlspecialchars ( $sql ) . "'", 'Execute' );
						return false;
					}
		...
		/**
		 *
		 * @return s an array with the primary key columns in it.
		 */
		 ...
		 *        	name case-insensitive (required by some databases)
	...
	class ADORecordSet implements IteratorAggregate {
		 ...
		 *
		 * @param int $upper
		 *        	Case for the array keys, defaults to uppercase
